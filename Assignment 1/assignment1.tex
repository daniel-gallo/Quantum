...
\begin{document}
    ...
    \begin{enumerate}
        ...
        \item First we have to find an orthonormal basis $\bar{\B} = \left\{\ket{1}, \dots, \ket{n}\right\}$ for $\H$. Then, an orthonormal basis for $\End(\H)$ (which has dimension $n^2$) is given by
        \begin{equation*}
            \B = \left\{\ket{i} \bra{j}\right\}_{i, j = 1}^n
        \end{equation*}
        To prove this, take two elements from $\B$, $\ket{i} \bra{j}$ and $\ket{k} \bra{l}$ and consider its inner product. We will change the notation for clarity's sake.
        \begin{equation*}
            \langle i j^\dagger, k l^\dagger\rangle = \tr (j \underbrace{i^\dagger k}_{\text{scalar}} l^\dagger)
        \end{equation*}
        If $i \neq k$ we get that $\ket{i} \bra{j}$ and $\ket{k} \bra{l}$ are orthogonal, as we wanted. If they are equal,
        \begin{equation*}
            \langle i j^\dagger, k l^\dagger\rangle = \tr (j l^\dagger) = \tr
                \begin{bmatrix}
                    j_{1} l_{1}^*&& \\
                    &\ddots& \\
                    &&j_{n} l_{n}^*
            ...
            = \braket{l | j}
        \end{equation*}
        If $l = j$ (and $i = k$), we get that $\langle i j^\dagger, k l^\dagger\rangle = 1$ and if  $l \neq j$ (and $i = k$), that $\langle i j^\dagger, k l^\dagger\rangle = 0$.
        ...
        \end{itemize}
        All basis elements have length 1, which can be easily seen by noting that $\langle M, M\rangle = \tr(M^\dagger M) = \sum_{i,j} \abs{M_{ij}}^2 = 1$. Now we have to prove that the elements are indeed orthogonal. The inner product of two different matrices withing the same class (created with the same rule) is going to be zero, because the non-zero elements are going to be in different positions. The same thing happens when computing the inner product of a matrix of the diagonal class with any other (different) matrix. If one of the matrices is in the $i < j$ class and the other in the $i > j$ class, two things could happen:
        \begin{itemize}
            \item If the non-zero elements are in different positions the inner product is going to be zero.
            \item If the non-zero elements are in the same positions, the inner product will be
            \begin{equation*}
                \langle A, B \rangle = \frac{1}{\sqrt{2}} \frac{i}{\sqrt{2}} - \frac{1}{\sqrt{2}} \frac{i}{\sqrt{2}} = 0
            \end{equation*} 
        \end{itemize}
    \end{enumerate}
    ...
    \begin{enumerate}
        \item Recall that the eigenvalues are obtained by solving $\det(\sigma_j - \lambda I) = 0$. To obtain the eigenvectors (the columns of the change of basis matrix) we have to find $\ker(\sigma_j - \lambda I)$. Since the Pauli matrices are Hermitian, we can diagonalize them.
        \begin{enumerate}
            \item
            \begin{equation*}
                \sigma_0 =
        ...
        \item
        \begin{itemize}
            \item If $j = k$, the commutator vanishes and $\epsilon_{jkl} = 0$ for all $l$ because there are two indices which are equal. Therefore the equation holds in that case. If $j$ and $k$ are different, note that $\left[\sigma_j, \sigma_k\right] = -\left[\sigma_k, \sigma_j\right]$ and that $\epsilon_{jkl} = -\epsilon_{kjl}$, since the parity of the permutation changes. Thus, we only have to check the following cases:
            \begin{align*}
                \left[\sigma_1, \sigma_2\right] &= 2i\sigma_3 \\
                \left[\sigma_1, \sigma_3\right] &= -2i\sigma_2 \\
                \left[\sigma_2, \sigma_3\right] &= 2i\sigma_1 \\
            \end{align*}
            \item If $j = k$ the equation holds because the Pauli matrices are involutory ($\sigma_j \sigma_j = I = \sigma_0$) and $\epsilon_{jjl} = 0$ for all $l$. If $j$ and $k$ are different, note that $\sigma_j \sigma_k = -\sigma_k \sigma_j$.
            \begin{align*}
                \sigma_1 \sigma_2 &= 
                \begin{bmatrix}
                    i & 0 \\
            ...
            Then,
            \begin{equation*}
                \left[\sigma_j, \sigma_k\right] = \sigma_j \sigma_k - \sigma_k \sigma_j = 2\sigma_j\sigma_k
            \end{equation*}
            Using the previous result, we conclude that if $j \neq k$, 
            \begin{equation*}
                \sigma_j\sigma_k = \frac{\left[\sigma_j, \sigma_k\right]}{2} = i\sum_{l=1}^3 \epsilon_{jkl} \sigma_l
            \end{equation*}.
        \end{itemize}
    \end{enumerate}
    ...
    \begin{enumerate}
        ...
        Let $A$ be a matrix representation of the positive operator $\hat{A}$. Since $\hat{A}$ is positive, $\hat{A}$ is Hermitian, and by the \textit{Spectral Theorem} we can express $A$ as follows
        \begin{equation*}
            A = P D P^{-1} = P
            \begin{bmatrix}
                \lambda_1&&& \\
        ...
        \begin{equation*}
            ...
            P^{-1}
        \end{equation*}
        As one can easily check, $\sqrt{A}\sqrt{A} = P \sqrt{D} P^{-1} P \sqrt{D} P^{-1} = P D P^{-1} = A$. Note that $\sqrt{A}$ is still positive because $\sqrt{\lambda_i} \geq 0$ for all $i$.
        \subsubsection*{Uniqueness}
        Let $B$ be the square root the above method gives us and let us assume that $C$ is a square root of $A$ as well. Now we construct a polynomial $p$ such that $p(\lambda_i) = \sqrt{\lambda_i}$ for every $\lambda_i$. Then, $p(D) = \sqrt{D}$ and
        \begin{equation*}
            p(C^2) = p(A) = p(P D P^{-1}) = P \sqrt{D} P^{-1} = B
        \end{equation*}
        Since we have expressed $B$ as a polynomial of $C^2$, we can see that $C$ and $B$ commute. This is useful because there is a well-known theorem in Lineal Algebra saying that we can simultaneously diagonalize two Hermitian matrices if they commute.
        \begin{align*}
            B = U \Lambda_1 U^{-1} \\
            C = U \Lambda_2 U^{-1}\\
        \end{align*}
        Now, since we know that $B^2 = C^2 = A$, we can see that $\Lambda_1^2 = \Lambda_2^2$, which means that $\Lambda_1 = \Lambda_2$, because $B$ and $C$ are positive operators. Thus, $B = C$, as we wanted.
    \end{enumerate}
\end{document}